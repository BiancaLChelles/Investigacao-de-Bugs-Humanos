...
const investigarBugHumano = (sintomaDetectado , intensidade, h = new Date().getHours()) => {
    ...
    const tardeNoite = (h >= 22 || madrugada);
        ...
        case "pensamento em looping":
            if (madrugada) {
                return {
                    ...
                    instrucao: "INTERRUPÇÃO FORÇADA: O sistema não resolve problemas após as 0h. Beba água e force o standby.",
                    frase: "Ficar rodando código com erro não conserta arquivo corrompido, principalmente às 3h da manhã, né? \n VOLTE Á REALIDADE!"
                };
            } else {
                return {
                    diagnostico: "Ruminação / Loop do Passado.",
                    explicacao: "Tentando editar logs de eventos que já foram fechados.",
                    instrucao: "O passado é apenas para leitura. Saia desse arquivo.",
                    frase: "Pare de rodar um script que só retorna erro!"
                };
            }
...
// EXEMPLO: investigarBugHumano ("sentimento" , intensidade , horario)

const meuStatusAgora = investigarBugHumano ("sede" , 5, 19);
